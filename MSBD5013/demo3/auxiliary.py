...
def generate_bar(data_list):
    ...
    '''
    import numpy as np
    open_price = data_list[0][:,0]
    high = np.max([array[:,1] for array in data_list],axis=0)
    low = np.min([array[:,2] for array in data_list],axis=0)
    close_price = data_list[-1][:,3]
    OHLC = np.array([open_price,high,low,close_price]).T
    
    return OHLC

def white_soider(data_cur, data_prev, asset=2):
    ''' Pattern for long signal: (unstrict) three white soliders
        For three sequential bars, if they have
        1. OPEN(n) > OPEN(n-1)
        2. CLOSE(n) > CLOSE(n-1)
        3. CLOSE(n) > OPEN(n)
        then we long 10 lots of AU.SHF at next bar unless the current cash balance 
        is less than 3,000,000
        
        stop loss point: When the price drop down to the close price of the first 
        white soilder, clear all long position
        
        target profit point: When the price go up to (1+5%) times the close price of 
        the third white soilder, clear all long position
        
        Params: data_cur - current minute data matrix
                data_prev - previous minute dta matrix
                asset - index of asset, here we use AU as default
    '''
...
def black_craw(data_cur, data_prev, asset=2):
    ''' Pattern for short signal: (unstrict) three black craws
        For three sequential bars, if they have
        1. OPEN(n) < OPEN(n-1)
        2. CLOSE(n) < CLOSE(n-1)
        3. CLOSE(n) < OPEN(n)
        then we short 10 lots of AU.SHF at next bar unless the current cash balance 
        is less than 3,000,000
        
        stop loss point: When the price go up to the close price of the first black 
        craw, clear all short position
        
        target profit point: When the price go up to (1-5%) times the close price of 
        the third black craw, clear all long position
